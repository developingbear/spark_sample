...
import common.domain.{AdLog, AtomLog, Identifier, Ranker, RetargetingMetrics}

import org.apache.spark.sql.functions._
import org.apache.spark.sql.{Column, DataFrame, Encoders}


class Core(logicalDateTimeStr: String) extends SparkBaseJob {

  import spark.implicits._

  private val logicalDateTime: LogicalDateTime = new LogicalDateTime(logicalDateTimeStr)

  private val udf = new CoreUDF()

  // Core functions

  def transformAtomRankerDF(idType: Identifier.Value, rankerLogType: Ranker.Value, atomRankerLogDF: DataFrame): DataFrame = {

    var transformedDF = transformCommonRankerLog(idType, atomRankerLogDF)
    transformedDF = transformAtomLog(idType, transformedDF)

    if(rankerLogType == Ranker.CONVERSION)
      transformedDF = transformedConversionLog(transformedDF)
  ...
  private def transformCommonRankerLog(idType: Identifier.Value, enrichLogDF: DataFrame): DataFrame = {
    val retargetingMetricSchema = Encoders.product[RetargetingMetrics].schema
    /**
     * enrich log format에서 ranker_raw_log에 json string에서 column으로 변환.
     * 최종 결과는 enrich log + ranker_raw_log 에서 추출한 cols
     */
    val tempDF = enrichLogDF
      .select(
        $"*",
    ...
    resultDF.select(
      $"*",
        ...
        .when($"ranker.rawMessage.winner.rankerType" === "ava", when(udf.strToMap($"ranker.rawMessage.rank.metrics")("atom") === "1", lit("true"))
          .otherwise(lit("false")))
        ...
        when($"ranker.rawMessage.action.actionType" === "win" || $"ranker.rawMessage.action.actionType" === "rimp",
          when($"ranker.rawMessage.winner.rankerType" === "danke", get_json_object($"ranker.rawMessage.winner.ranking.ad", "$.pctr").cast("Double"))
            .when($"ranker.rawMessage.winner.rankerType" === "ava", udf.strToMap($"ranker.rawMessage.winner.ranking.ad")("p").cast("Double"))
            .otherwise(lit(0.0))
      ...
      .withColumn("retProdCnt",
        when($"actionType" === "vimp",
          when($"pms".isNotNull, udf.getRetargetProductCount($"pms"))
            .otherwise(lit(0))
        ).otherwise(lit(0)))
  ...
  private def transformAtomLog(idType: Identifier.Value, atomClickDF: DataFrame): DataFrame = {
    ...
    resultDF.select(
        ...
        $"atom.stateInfoDttm",
        $"atom.tes",
        udf.convertIdTypeBitToReadable($"atom.tes") as "tagExistStatus",
        $"atom.ies",
        udf.convertIdTypeBitToReadable($"atom.ies") as "idExistStatus",
        $"atom.tcom",
        $"atom.tccm",
        $"atom.tcrm",
        $"atom.hsm",
        ...
        udf.mapToTagMap($"atom.survivals"($"adGroupId")("matchTag")) as "matchTags",
  ...
  private def transformedConversionLog(transformedDF: DataFrame): DataFrame = {
    transformedDF
//      .withColumn("")
      .select(
        $"*",
  ...
  private def transformAtomLogWithConv(atomClickDF: DataFrame): DataFrame = {
    ...
    val tempDF = atomClickDF
      ...
      .withColumn("tag_exist_status", udf.convertIdTypeBitToReadable($"atom.tes"))
      .withColumn("id_exist_status", udf.convertIdTypeBitToReadable($"atom.ies"))
  ...
  def writeRawMetric(idType: Identifier.Value,
                     rankerLogType: Ranker.Value,
                     rawMetricDF: DataFrame, repartitionNum: Int): Unit = {
    val savePath: String = idType match {
      case Identifier.ADID => s"/ns2/bizrec/lookalike/atom_log/metric/core/atom_${rankerLogType}/prd/date=${logicalDateTime.getDate()}/hour=${logicalDateTime.getHour()}"
      case Identifier.ACID => s"/ns2/bizrec_acid/lookalike/atom_log/metric/core/atom_${rankerLogType}/prd/date=${logicalDateTime.getDate()}/hour=${logicalDateTime.getHour()}"
    }

    var filteredDF = rawMetricDF
    if(rankerLogType == Ranker.WIN)
  ...
  def writeMetric(idType: Identifier.Value,
                  rankerLogType: Ranker.Value,
                  rawMetricDF: DataFrame): Unit = {

    val savePath = idType match {
      case Identifier.ADID =>
        s"/ns2/bizrec/lookalike/atom_log/metric/druid/atom_${rankerLogType}/prd/date=${logicalDateTime.getDate()}/hour=${logicalDateTime.getHour()}"
      case Identifier.ACID =>
        s"/ns2/bizrec_acid/lookalike/atom_log/metric/druid/atom_${rankerLogType}/prd/date=${logicalDateTime.getDate()}/hour=${logicalDateTime.getHour()}"
    }

    if (idType == Identifier.ADID && rankerLogType != Ranker.CONVERSION) {
      rawMetricDF
